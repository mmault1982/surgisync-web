import {
  FILTERABLE_COLUMNS,
  activeFilterKeys,
  type FilterKey,
  type OnHandSearch,
} from '../on-hand.search';

/**
 * One chip per active filter, each clearing only itself.
 *
 * The prototype clears the whole COLUMN when a chip is dismissed — including
 * that column's sort — which is more than the chip claims to do.
 */
export function ActiveFilterChips({
  search,
  onChange,
  onClearAll,
}: {
  search: OnHandSearch;
  onChange: (patch: Partial<OnHandSearch>) => void;
  onClearAll: () => void;
}) {
  const keys = activeFilterKeys(search);
  if (keys.length === 0 && !search.search) return null;

  return (
    <div className="mb-3 flex flex-wrap items-center gap-2 rounded-lg border border-[#FEE8EB] bg-[#fff5f6] px-3 py-2">
      <span className="text-[11px] font-bold uppercase tracking-wide text-brand">
        Active filters
      </span>

      {search.search && (
        <Chip
          label={`Search: "${search.search}"`}
          onRemove={() => onChange({ search: undefined })}
        />
      )}

      {keys.map((key) => (
        <Chip
          key={key}
          label={`${FILTERABLE_COLUMNS[key]}: ${describe(key, search)}`}
          onRemove={() => onChange({ [key]: undefined })}
        />
      ))}

      <button
        type="button"
        onClick={onClearAll}
        className="ml-auto rounded-full border border-brand px-2.5 py-1 text-xs font-semibold text-brand hover:bg-brand hover:text-white"
      >
        Clear all
      </button>
    </div>
  );
}

function describe(key: FilterKey, search: OnHandSearch): string {
  const value = search[key];
  if (Array.isArray(value)) return `${value.length} selected`;
  if (typeof value === 'boolean') return value ? 'Yes' : 'No';
  return String(value);
}

function Chip({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-brand bg-white px-2.5 py-1 text-xs text-brand">
      {label}
      <button type="button" onClick={onRemove} aria-label={`Remove filter ${label}`}>
        ✕
      </button>
    </span>
  );
}
